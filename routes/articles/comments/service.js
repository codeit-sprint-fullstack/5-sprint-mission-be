...
const getComments = async (req, res) => {
  try {
    const articleId = req.params.domainId;
    ...
    const comments = await prisma.articlesComments.findMany({
      where: {
        articleId,
        deletedAt: null,
      },
...
const createComment = async (req, res) => {
  try {
    ...
    const newComment = await prisma.articlesComments.create({
      data: {
...
const patchComment = async (req, res) => {
  try {
    ...
    // 댓글 존재 여부 확인
    const existingComment = await prisma.articlesComments.findUnique({
      where: {
        id,
        deletedAt: null,
      },
    ...
    const updatedComment = await prisma.articlesComments.update({
      where: { id },
      data: { content },
    });
...
const deleteComment = async (req, res) => {
  try {
    ...
    //deletedAt 업데이트
    const deletedComment = await prisma.articlesComments.update({
      where: { id },
      data: {
        deletedAt: new Date(),
      },
